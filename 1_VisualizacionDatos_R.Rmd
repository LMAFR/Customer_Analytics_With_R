...
p + geom_point(aes(displ, hwy, shape = factor(year), size = cyl))

# Para establecer la forma (shape) no se puede usar una variable continua, por eso lo pasamos a factor.
...
ggplot(data = mpg, mapping = aes(x = displ, y = hwy)) + 
  ...
  geom_smooth(se=T) +    # se = confidence interval, sombreado
...
ggplot(mpg, aes(cty,hwy)) +
  geom_point(aes(color = drv)) + 
  geom_smooth(method = "lm") + 
  geom_hline(yintercept =  30, color = 'blue', linetype = 3)  + 
  theme_minimal()
